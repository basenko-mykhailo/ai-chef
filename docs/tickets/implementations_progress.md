...
| 2. Family members | 🟡 partial | 2.1 migration + 2.2 model + 2.3 list page done; 2.4–2.5 pending |
...
- ⬜ **2.4 Add/edit form** — no controller/form/view for create or update of family members.
...
Continue Epic 2 with ticket 2.4 — add the create/edit form for family members. This will need a `FamilyMemberController@create/store/edit/update` set, a Blade form view with three textareas (favorite / disliked / allergies & diets), and a swap of the `#` placeholder hrefs in `family/index.blade.php` for `route('family.create')` / `route('family.edit', $member)`. Then 2.5 (delete with confirmation). After Epic 2 is closed, start Epic 1 (pantry) — it's the bigger unblocker for Epics 3/4/6. Still worth flipping `APP_LOCALE=en` → `uk` in `src/.env` before deeper UI work.